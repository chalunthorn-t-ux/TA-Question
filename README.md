...
## จัดการไฟล์ความรู้จากหน้าเว็บ

หน้า **`/admin/knowledge`** (เข้าจากลิงก์ “จัดการคลังความรู้” ในแถบซ้าย) รวมงานจัดการเอกสารไว้ที่เดียว
โดยไม่ต้องเปิดเครื่องรันสคริปต์ — ใช้ได้ทั้งตอนรันในเครื่องและตอนขึ้น Vercel

| ต้องการทำ | วิธีทำ |
|---|---|
| ดูว่าตอนนี้มีเอกสารอะไรอยู่ | เปิดหน้านี้ ตารางบอกขนาด เวลาแก้ล่าสุด และจำนวน chunk ใน index |
| เพิ่มเอกสารใหม่ | อัปโหลด แล้วกด **อัปเดต Index** |
| **แก้ไขเอกสารเดิม** | กด **ดาวน์โหลด** เอาไปแก้ → อัปโหลดกลับด้วย **ชื่อไฟล์เดิม** (ระบบเขียนทับให้) → กด **อัปเดต Index** |
| ลบเอกสาร | กด **ลบ** — เอาออกทั้งจากคลังและจาก index ในคราวเดียว |
| ล้างคลังแล้วสร้างใหม่ทั้งหมด | กด **สร้างใหม่ทั้งหมด** (ใช้ตอน index มีเศษของเอกสารเก่าปนอยู่) |

> **ทำไมต้องกด “อัปเดต Index” ทุกครั้ง** — index เก็บข้อความที่ตัดเป็น chunk พร้อมเวกเตอร์ไว้ในตัวเอง
> ไม่ได้อ่านไฟล์ต้นฉบับตอนตอบคำถาม แก้ไฟล์อย่างเดียวระบบจึงยังตอบด้วยเนื้อหาเก่า

ไฟล์ต้นฉบับถูกเก็บไว้ต่างที่กันตามสภาพแวดล้อม แต่หน้านี้ใช้งานเหมือนกันทั้งคู่

- รันในเครื่อง (เขียนดิสก์ได้) → โฟลเดอร์ `data/`
- บน Vercel (ดิสก์เขียนไม่ได้) → Vercel Blob หรือ Postgres ใต้ `docs/`

รายการ **“อยู่ใน index แต่ไม่มีไฟล์ต้นฉบับ”** คือเอกสารที่ระบบยังตอบได้แต่ไม่มีไฟล์ให้แก้แล้ว
(เช่น index ถูกสร้างในเครื่องแล้ว push ขึ้นไป) — อัปโหลดไฟล์ชื่อเดิมเพื่อกู้คืน หรือเอาออกจาก index ถ้าไม่ใช้แล้ว

### ข้อจำกัดตอนใช้บน Vercel

| เรื่อง | ขีดจำกัด | ทำยังไงถ้าชน |
|---|---|---|
| ขนาดไฟล์ต่อการอัปโหลด | ~4.5 MB (เพดานของ serverless function ไม่ใช่ของแอป) | อัปทีละไฟล์ หรือย่อ/แยกไฟล์ก่อน |
| เวลาต่อคำขอ | 60 วินาที (`maxDuration` ใน `vercel.json`) | ถ้า “อัปเดต Index” ไม่ทัน ให้ทำในเครื่องแทน (ดูด้านล่าง) |
| ต้องมีที่เก็บถาวร | `BLOB_READ_WRITE_TOKEN` หรือ `DATABASE_URL` | ไม่มีเลย หน้านี้จะขึ้นเตือนและปิดปุ่มให้ ไม่ล้มทั้งหน้า |

ถ้าเอกสารเยอะจนทำผ่านเว็บไม่ทันเพดานเวลาของ serverless ให้ทำในเครื่องแทน:
...
| `GET` | `/admin/knowledge` | จัดการไฟล์ความรู้: ดู เพิ่ม แทนที่ ดาวน์โหลด ลบ (admin) |
| `POST` | `/api/sources/remove` | เอาเอกสารหนึ่งไฟล์ออกจาก index (admin) |
| `POST` | `/api/upload` | อัปโหลดไฟล์เข้าคลังเอกสาร (multipart) |
