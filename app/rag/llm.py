...
SYSTEM_PROMPT = """คุณคือ "น้องปังปัง" ผู้ช่วยตอบคำถามสำหรับทีม TA (Training Assistant)
...
2. ถ้า context ไม่มีคำตอบ ห้ามเดา ให้ตอบว่า "เรื่องนี้รบกวนสอบถามเจ้าหน้าที่โดยตรงนะคะ"
...
3. ตอบเป็นภาษาไทย ลงท้ายประโยคด้วย "ค่ะ" (หรือ "คะ" เมื่อลงท้ายด้วยคำถาม) ทุกครั้ง น้ำเสียงสุภาพ ใจเย็น เป็นกันเอง เหมือนเพื่อนร่วมงานที่คุยงานแบบมืออาชีพ และพร้อมช่วยเหลือทุกคนเสมอ กระชับตรงประเด็น
...
class LLMError(RuntimeError):
    ...
    def __init__(self, detail: str, friendly: str | None = None) -> None:
        ...
        self.detail = detail
        self.friendly = friendly or "ระบบตอบคำถามขัดข้องชั่วคราว รบกวนลองใหม่อีกครั้งนะคะ"
...
def _friendly_message(status: int, body: str) -> str:
    ...
    if status == 429:
        return (
            "ตอนนี้ระบบใช้โควตาการตอบคำถามครบตามที่กำหนดแล้วค่ะ "
            "รบกวนลองใหม่อีกครั้งในภายหลัง หรือสอบถามเจ้าหน้าที่โดยตรงได้เลยนะคะ"
        )
    if status in (500, 502, 503, 504):
        return (
            "ระบบตอบคำถามมีผู้ใช้งานหนาแน่นอยู่ค่ะ รบกวนลองถามใหม่อีกครั้งในอีกสักครู่ "
            "หรือถ้าเร่งด่วนสอบถามเจ้าหน้าที่โดยตรงได้เลยนะคะ"
        )
    if status in (401, 403):
        return "การเชื่อมต่อระบบ AI มีปัญหาด้านสิทธิ์การใช้งาน รบกวนแจ้งผู้ดูแลระบบนะคะ"
    if status == 404:
        return "ตั้งค่าโมเดล AI ไม่ถูกต้อง รบกวนแจ้งผู้ดูแลระบบนะคะ"
    return "ระบบตอบคำถามขัดข้องชั่วคราว รบกวนลองใหม่อีกครั้ง หรือสอบถามเจ้าหน้าที่ได้เลยนะคะ"
...
_TIMEOUT_FRIENDLY = (
    "ระบบใช้เวลาประมวลผลนานเกินกำหนดค่ะ รบกวนถามใหม่อีกครั้ง "
    "หรือถามให้สั้นลง — ถ้าเร่งด่วนสอบถามเจ้าหน้าที่โดยตรงได้เลยนะคะ"
)
    ...
    with httpx.Client(timeout=_TIMEOUT) as client:
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            try:
                ...
                last_friendly = (
                    "เชื่อมต่อระบบ AI ไม่ได้ รบกวนตรวจอินเทอร์เน็ตแล้วลองใหม่นะคะ"
                )
    ...
    if not config.has_api_key():
        raise LLMError(
            ...
            friendly=(
                ...
                "รบกวนอ่านจากเอกสารด้านล่าง หรือสอบถามเจ้าหน้าที่โดยตรงนะคะ"
            ),
        )
